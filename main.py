...
ANALYZE_EVERY_N_FRAMES = 8  # bigger -> faster FPS, less frequent analysis
...
def draw_result(frame, result):
    try:
        ...
        if 'age' in res and res['age'] is not None:
            info_parts.append(f"Age:{int(res['age'])}")
        if 'dominant_gender' in res and res['dominant_gender']:
            info_parts.append(str(res['dominant_gender']))
        if 'dominant_emotion' in res and res.get('dominant_emotion'):
            info_parts.append(str(res['dominant_emotion']))
